...
class Embedder(torch.nn.Module):
    ...
    def __init__(self, embedding_size, name="", initializer=None, vocabulary=None, num_tokens=-1, anonymizer=None, freeze=False, use_unk=True):
        ...
        if vocabulary:
            assert num_tokens < 0, "Specified a vocabulary and set the number of tokens to " + \
                str(num_tokens)
            ...
            def check_vocab(index):
                ...
                assert index < num_tokens, "Passed token ID of " + \
        ...
        print("Creating a token embedder called " + emb_name + ", whose size is" + str(self.vocabulary_size) + " x " + str(embedding_size))
        ...
        if self.anonymizer:
            ...
            print("Creating an entity embedder called " + emb_name + ", whose size is" + str(entity_size) + " x " + str(embedding_size))
    ...
    def forward(self, token):
        assert isinstance(token, int) or not snippet_handler.is_snippet(token), "Embedders should only be called on flat tokens; Use snippet_bow if you are trying to encode snippets"
